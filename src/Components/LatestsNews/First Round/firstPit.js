...
const FirstPit = () => {
    ...
    useEffect(() => {
        fetch(`http://lookup-service-prod.mlb.com/json/named.leader_pitching_repeater.bam?sport_code='mlb'&results=10&game_type='F'&season='${new Date().getFullYear()}'&sort_column=era`)
        .then(res => res.json())
        .then(data =>{
