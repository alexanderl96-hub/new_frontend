...
const SpringPit = () => {
    ...
    useEffect(() => {
        fetch(`http://lookup-service-prod.mlb.com/json/named.leader_pitching_repeater.bam?sport_code='mlb'&results=10&game_type='S'&season='${new Date().getFullYear()}'&sort_column=era`)
        .then(res => res.json())
        .then(data =>{
